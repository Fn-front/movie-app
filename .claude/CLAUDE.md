...
**【推奨】Worktree使用ルール**：
- 複数Issue/ブランチを並行作業する場合は `isolation: "worktree"` を使用する
- Worktreeを使うことでmainの作業ツリーを汚さずに独立したブランチで作業可能
- 例: Agentツールで `isolation: "worktree"` を指定して別Issueを並行処理

**Worktreeの使い分け**：
- 独立したIssue（依存関係なし）→ Worktreeで並行作業
- 依存関係のあるIssue → 同一ブランチで順次対応

### Background Agents活用

**【推奨】バックグラウンド実行**：
- 長時間タスク（全テスト実行、E2Eテスト、CI結果待ち等）は `run_in_background: true` で実行する
- バックグラウンドタスクは完了時に自動通知されるため、ポーリングやsleepは不要
