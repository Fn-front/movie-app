...
**【必須】Issue起票の観点・運用（Claude Code向け）**：
- Issueは**確認を取らず自律的に作成してよい**。ただし**作成したら必ず、タスクとしてagentにそのIssueをレビューさせる**（内容の正確性・スコープ・抜け漏れを点検）
- 起票トリガー：(1) 検証中に見つけたバグ（再現手順＋根拠を本文に）/ (2) テスト不足（単体・バリデーション等）/ (3) **ストーリー/ジャーニー単位で足りないテスト**（異常系なし・途中ステップ未検証・E2E欠如）/ (4) 仕様判断が要る点（`question`）/ (5) 作業中に派生した改善（本筋と分けて別Issue）/ (6) セキュリティ
- 粒度：**1関心事=1Issue**（別原因/別システムは分割）。「ついで」で広げない。重複は事前確認
- 依存関係は本文に明記（「前提: #xxx」）
- 本文の型：概要 / 原因（バグは再現＋根拠）/ 影響範囲 / 対応方針（候補＋推奨）/ 完了条件 / 関連Issue

**【必須】マージ承認ルール**：
- PR作成後は、そこで一旦停止しユーザーの**明示的な許可**を待つ
- 許可があるまで `gh pr merge` を実行しない（自動マージ禁止）
- 「マージしてよいか」を確認し、承認を得てからマージへ進む
...
**作業フロー**：
1. ブランチ作成 → 2. 実装 → 3. テスト作成 → 4. コミット → 5. ロードマップ更新 → 6. PR作成 → 7. **マージ承認待ち** → 8. マージ → 9. ブランチ削除
...
### ローカル動作確認（ブラウザ）— Claude Code向けルール

**【必須】Next.jsエラーバッジの確認**：
- スクリーンショットで動作確認するときは、画面**左下のNext.js Dev Toolsバッジ**を必ず確認する
