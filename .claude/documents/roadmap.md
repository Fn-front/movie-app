...
1. usersテーブルにroleカラム追加 + 管理者Seeder                                                                
  2. 新規登録フロー（フォーム + API + パスワードハッシュ化）                                                     
  3. メール送信 + OTP検証                                                                                        
  4. ログイン・セッション管理
  5. パスワード管理機能（リセット・変更）                                                                        
  6. セキュリティ対策（レート制限、CSRF、XSS）                                                                 
  7. UX改善（ローディング、エラーメッセージ、自動遷移）
...
- [ ] axiosインスタンスでAPI呼び出し
- [ ] 未認証アカウント自動削除
  - 登録から1時間以内にメール認証しないアカウントを物理削除
  - 未認証メールでの再登録時はパスワード上書き＋OTP再発行
