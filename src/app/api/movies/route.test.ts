...
describe('GET /api/movies', () => {
  ...
  it('認証済みユーザーでfavoritesが空の場合、全映画にfavorite: nullが付与される', async () => {
    const handler = await loadGET();
    ...
    expect(response.status).toBe(200);
    expect(json.data.movies[0].favorite).toBeNull();
    expect(json.data.movies[1].favorite).toBeNull();
  });

  it('キャッシュ期限切れ時にexistingOtherTypeがnullの場合でも正常に処理される', async () => {
    const handler = await loadGET();
    ...
    mockSupabaseClient!.from.mockImplementation(() => {
      ...
      // カウントクエリ
      const chain = createChainMock({ count: 0 });
      chain.select.mockImplementation(
        (_cols: string, opts?: { count?: string }) => {
          if (opts?.count === 'exact') {
            return createThenableProxy(createChainMock({ count: 0 }), {
              count: 0,
            });
          }
          return createThenableProxy(
  ...
  it('TMDb APIの総ページ数と同じページに達した場合、ループを終了する', async () => {
    const handler = await loadGET();

    // total_pages: 2 で、2ページ分の結果を返す
    mockDiscoverMovies
    ...
    const response = await handler(createRequest());
    expect(response.status).toBe(200);
    // total_pages=2なのでp=2でbreak → discoverは2回呼ばれる
