...
describe('syncNowPlayingMovies', () => {
  ...
  describe('nullフォールバックのブランチカバレッジ', () => {
    it('existingStreamingがnullの場合でも正常に動作する', async () => {
      setValidEnv();

      const movie = createMockMovie({ id: 900, title: 'ヌルテスト映画' });
      ...
      // select().neq() が data: null を返す（?? [] のフォールバックをテスト）
      const fromMock = jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue({
    ...
    it('release_dateが空文字の映画はnullとしてupsertされる', async () => {
      setValidEnv();
    ...
    it('clearedDataがnullの場合、cleared数は0になる', async () => {
      setValidEnv();
      ...
      // クリア処理で data: null を返す（clearedData?.length ?? 0 のフォールバックをテスト）
      const fromMock = jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue({
          neq: jest.fn().mockResolvedValue({ data: [] }),
        }),
